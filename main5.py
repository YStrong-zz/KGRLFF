...
from models.kgcn1_BiRW_opt1 import KGCN
from config5 import ModelConfig, PROCESSED_DATA_DIR,  ENTITY_VOCAB_TEMPLATE, PATH_VOCAB_TEMPLATE,\
    RELATION_VOCAB_TEMPLATE, ADJ_ENTITY_TEMPLATE, ADJ_RELATION_TEMPLATE, ADJ_PATH_TEMPLATE, \
    ADJ_ENTITY_ALL_INFO_TEMPLATE, LOG_DIR, PERFORMANCE_LOG, DRUG_VOCAB_TEMPLATE, SMILE_HASH, SMILE_FP,\
    INDEXES_RANK_TEMPLATE
...
def train(train_d,dev_d,test_d,kfold,dataset, neighbor_sample_size, embed_dim, n_depth, l2_weight, lr, optimizer_type,
    ...
    config.relation_vocab_size = len(pickle_load(format_filename(PROCESSED_DATA_DIR,
                                                                 ...
                                                                 dataset=dataset)))
    config.path_vocab_size = len(pickle_load(format_filename(PROCESSED_DATA_DIR,
                                                                 PATH_VOCAB_TEMPLATE,
                                                                 dataset=dataset)))

    config.adj_entity = np.load(format_filename(PROCESSED_DATA_DIR, ADJ_ENTITY_TEMPLATE, dataset=dataset))
    config.adj_relation = np.load(format_filename(PROCESSED_DATA_DIR, ADJ_RELATION_TEMPLATE, dataset=dataset))
    config.adj_path = np.load(format_filename(PROCESSED_DATA_DIR, ADJ_PATH_TEMPLATE, dataset=dataset))
    
#    config.adj_index_rank = np.load(format_filename(PROCESSED_DATA_DIR, INDEXES_RANK_TEMPLATE, dataset=dataset))
#    config.smile_fp = np.load(format_filename(PROCESSED_DATA_DIR, SMILE_FP, dataset=dataset))
#    config.smile_hash = np.load(format_filename(PROCESSED_DATA_DIR, SMILE_HASH, dataset=dataset), allow_pickle=True)

    config.exp_name = f'kgcn_{dataset}_neigh_{neighbor_sample_size}_embed_{embed_dim}_depth_' \
                      f'{n_depth}_agg_{aggregator_type}_optimizer_{optimizer_type}_lr_{lr}_L2_{l2_weight}_' \
                      f'batch_size_{batch_size}_epoch_{n_epoch}'
    experiment_name = f'kgcn_{dataset}_neigh_{neighbor_sample_size}_embed_{embed_dim}_depth_' \
                      f'{n_depth}_agg_{aggregator_type}_optimizer_{optimizer_type}_lr_{lr}_L2_{l2_weight}_' \
                      f'batch_size_{batch_size}_epoch_{n_epoch}_kfold_{kfold}'
    ...
    print('Logging Info - Experiment: %s' % experiment_name)
    ...
    auc, acc, f1,aupr, fpr, tpr, precision, recall= model.score(x=[valid_data[:, :1], valid_data[:, 1:2]], y=valid_data[:, 2:3])
    print('Scoring had completed...')
    ...
    model.load_best_model()
    auc, acc, f1, aupr, fpr, tpr, precision, recall = model.score(x=[test_data[:, :1], test_data[:, 1:2]], y=test_data[:, 2:3])
